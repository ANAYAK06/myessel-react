...
const VerifySupplierPO = ({ notificationData, onNavigate }) => {
    ...
    useEffect(() => {
        ...
        if (roleId && uid && roleId !== selectedRoleId) {
            ...
            console.log('âŒ Skipping API call. Missing values:', {
    ...
    useEffect(() => {
        if (selectedPOData?.MOID && roleId) {
            ...
            console.log('âŒ Skipping Status List fetch:', {
            ...
            if (checkPriceDifference(item)) {
                ...
                showPriceUpdateConfirmation(
                    ...
                    () => {
                        ...
                        console.log('âŒ Item Check Cancelled due to Price Update Refusal:', itemCode);
                    }
                );
            } else {
    ...
    const onActionClick = async (action) => {
        ...
        if (!selectedPO) {
            console.log('âŒ No PO selected');
            toast.error('No PO selected');
            return;
        }

        if (!verificationComment || verificationComment.trim() === '') {
            console.log('âŒ No verification comment provided');
        ...
        if (!areAllItemsChecked()) {
            console.log('âŒ Not all items checked');
        ...
        try {
            ...
            console.error(`âŒ PO ${action.type} Error:`, error);
            let errorMessage = `Failed to ${action.text.toLowerCase()}`;

            if (error && typeof error === 'string') {
                errorMessage = `âŒ ${error}`;
            } else if (error?.message) {
                errorMessage = `âŒ ${error.message}`;
            }
    ...
    // Card rendering for each PO in the left panel list
    const renderItemCard = (po) => {
            ...
            <div className="p-4">
                <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 dark:text-white text-sm truncate">{po.VendorName}</h3>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{po.VendorCode}</p>
                    </div>
                ...
                <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-gray-500 text-xs">Ref: {po.RefNo || 'N/A'}</span>
                    </div>
                </div>
            </div>
        );
        ...
        return (
            <div className="space-y-6">
                ...
                {selectedPOData.VendorAddress && (
                    <div className={`p-5 rounded-xl border ${isPopup ? 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700' : 'bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-700'}`}>
                        <h4 className={`font-semibold mb-4 flex items-center ${isPopup ? 'text-gray-800 dark:text-gray-200' : 'text-green-800 dark:text-green-200'}`}>
                                                                    ...
                                                                    <p className="font-semibold text-gray-900 dark:text-gray-100 text-xs leading-tight mb-0.5">
                                                                        ...
                                                                        {!isPopup && hasPriceDifference && (
                                                                            <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-300">
                                                                                <DollarSign className="w-3 h-3 mr-1" />Price Diff
                                                                            </span>
                                                                        )}
                                                                        {!isPopup && hasRecentChange && (
                                                                            <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300">
                                                                                <TrendingDown className="w-3 h-3 mr-1" />Recent Change
                                                                            </span>
                                                                        )}
                                                                        {!isPopup && isStandardPriceUpdated && (
                                                                            <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300">
                                                                                <CheckCircle className="w-3 h-3 mr-1" />Updated
                                                                            </span>
                                                                        )}
                                                                    </p>
                                                                    <div className={`absolute left-0 mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 whitespace-nowrap max-w-xs ${index === 0 ? 'top-full mt-2' : 'bottom-full mb-2'}`}>
                                                                        ...
                                                                        <div className={`absolute left-4 w-0 h-0 border-l-4 border-r-4 border-transparent ${index === 0 ? 'bottom-full border-b-4 border-b-gray-900' : 'top-full border-t-4 border-t-gray-900'}`}></div>
                                                                    </div>
                                                                </div>
                                                                <button
                                                                    onClick={() => handleItemCodeClick(item.itemcode)}
                                                        ...
                                                        <div className={isPopup ? '' : 'space-y-1'}>
                                                            <div className="flex items-center justify-center space-x-1">
                                                                ...
                                                                <input
                                                                    ...
                                                                    title={isChecked ? 'Price locked after verification' : `Current: ₹${formatIndianCurrency(currentEditablePrice)} | Standard: ₹${formatIndianCurrency(item.basicprice)}`}
                                                                />
                ...
                {selectedPOData.Remarks && (
                    <div className={`p-6 rounded-xl border ${isPopup ? 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700' : 'bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-orange-900/20 dark:to-yellow-900/20 border-orange-200 dark:border-orange-700'}`}>
                        <h4 className={`font-semibold mb-4 flex items-center ${isPopup ? 'text-gray-800 dark:text-gray-200' : 'text-orange-800 dark:text-orange-200'}`}>
                ...
                <div className={`p-4 rounded-xl border border-gray-200 dark:border-gray-700 ${isPopup ? 'bg-white dark:bg-gray-800' : 'bg-gradient-to-br from-gray-50 to-indigo-50 dark:from-gray-900/20 dark:to-indigo-900/20'}`}>
                    <button
                ...
                <div className={`p-5 rounded-xl border-2 ${isPopup ? 'bg-white dark:bg-gray-800 border-red-200 dark:border-red-700' : 'bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 border-red-200 dark:border-red-700'}`}>
