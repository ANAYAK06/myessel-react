...
import { Lock, Clock, Calendar, FileText, Loader2 } from 'lucide-react';
...
import {
    ...
    selectHPVList,
    selectHPVDetail,
    selectHPVListLoading,
    selectHPVDetailLoading,
    selectHPVApproveLoading,
    selectHPVListError,
} from '../../slices/accountsSlice/holdPaymentVerificationSlice';
...
const VerifyHoldPayment = ({ notificationData, onNavigate }) => {
    const dispatch = useDispatch();

    const list           = useSelector(selectHPVList);
    const detail         = useSelector(selectHPVDetail);
    const listLoading    = useSelector(selectHPVListLoading);
    const detailLoading  = useSelector(selectHPVDetailLoading);
    const approveLoading = useSelector(selectHPVApproveLoading);
    const listError      = useSelector(selectHPVListError);
    ...
    useEffect(() => {
        if (roleId) dispatch(fetchHPVList(roleId));
        dispatch(setShowReturnButton('Yes'));
        return () => {
            dispatch(resetHPVVerification());
    ...
    useEffect(() => {
        ...
        dispatch(fetchHPVDetail(selectedItem.BankTransactionRefNo));
        setIsVerified(false);
    ...
    const handleRefresh = () => { if (roleId) dispatch(fetchHPVList(roleId)); };

    const handleBackToInbox = () => {
        if (onNavigate) onNavigate('dashboard', { name: 'Dashboard', type: 'dashboard' });
    };

    const invoiceRows = detail?.HoldInvoiceList || [];

    // spApproveHoldPayment parses these with `WHILE (charindex(',',@Invoicenos) <> 0)`,
    ...
    const handleActionClick = async (action) => {
        ...
        try {
            const result = await dispatch(approveHPV({
                ...
                PaidHoldAmounts: joinWithTrailingComma(invoiceRows.map((r) => r.HoldBalance)),
                Action:          actionValue,
            ...
            setTimeout(() => {
                dispatch(fetchHPVList(roleId));
                setSelectedItem(null);
                setVerificationComment('');
                setIsVerified(false);
                setShowRemarksHistory(false);
                setIsLeftPanelCollapsed(false);
                dispatch(clearHPVDetail());
                dispatch(resetApprovalData());
                dispatch(clearHPVApproveResult());
            }, 800);
        } catch (err) {
    ...
    const renderItemCard = (item) => (
        <div className="p-4">
            <div className="flex items-center gap-3 mb-3">
                <div className="w-11 h-11 rounded-full border-2 border-rose-200 dark:border-rose-600 bg-gradient-to-br from-rose-100 to-pink-100 dark:from-rose-800/40 dark:to-pink-800/40 flex items-center justify-center shrink-0">
            ...
            <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold text-rose-600 dark:text-rose-400">{fmt(item.PaymentAmount)}</span>
            </div>
        </div>
    );
    ...
    const renderListItem = (item) => (
        <div className="flex items-center gap-x-6 gap-y-1 flex-wrap text-sm">
            ...
            <span className="ml-auto font-bold text-rose-600 dark:text-rose-400 whitespace-nowrap">{fmt(item.PaymentAmount)}</span>
        </div>
    );

    const renderCollapsedItem = () => (
        <div className="w-full h-full rounded-lg border-2 border-rose-200 dark:border-rose-600 bg-gradient-to-br from-rose-100 to-pink-100 dark:from-rose-800/40 dark:to-pink-800/40 flex items-center justify-center">
    ...
    const renderDetailContent = () => {
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-rose-50 to-pink-50 dark:from-rose-900/20 dark:to-pink-900/20 rounded-2xl p-6 border-2 border-rose-200 dark:border-rose-700">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-rose-600 dark:text-rose-400 font-semibold text-sm mt-0.5">Hold Payment</p>
                            <div className="flex flex-wrap gap-2 mt-3">
                                <span className="px-3 py-1 bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 rounded-full text-xs font-medium">
                ...
                {invoiceRows.length > 0 && (
                    <div className="rounded-xl border-2 border-rose-200 dark:border-rose-800 bg-rose-50/50 dark:bg-rose-900/10 p-4">
                        <p className="text-xs font-bold uppercase tracking-wide mb-3 text-rose-700 dark:text-rose-400 flex items-center gap-1.5">
                            ...
                            <table className="w-full text-xs">
                                <thead>
                                    <tr className="bg-rose-100/60 dark:bg-rose-900/20">
                                        <th className="px-3 py-2 text-left font-bold text-rose-700 dark:text-rose-300 uppercase">Invoice No</th>
                                        <th className="px-3 py-2 text-left font-bold text-rose-700 dark:text-rose-300 uppercase">PO Number</th>
                                        <th className="px-3 py-2 text-left font-bold text-rose-700 dark:text-rose-300 uppercase">Date</th>
                                        <th className="px-3 py-2 text-right font-bold text-rose-700 dark:text-rose-300 uppercase">Hold</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {invoiceRows.map((row, i) => (
                                        <tr key={row.ClientInvoiceNo || i} className="border-t border-rose-100 dark:border-rose-900/30">
                                            ...
                                            <td className="px-3 py-2 text-right font-semibold">{fmt(row.HoldBalance)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <VerificationInput
                    ...
                    config={{
                        checkboxLabel: '✓ I have verified this hold payment',
                        ...
                        commentGradient: 'from-rose-50 to-pink-50 dark:from-rose-900/20 dark:to-pink-900/20',
                        commentBorder: 'border-rose-200 dark:border-rose-700',
                    }}
                />
    ...
    return (
        <div className="space-y-6">
            <InboxHeader
                title={`${InboxTitle || 'Hold Payment Verification'} (${list.length})`}
                subtitle={ModuleDisplayName}
                itemCount={list.length}
                onBackClick={handleBackToInbox}
                HeaderIcon={Lock}
                badgeText="Hold Payment"
            ...
            <InboxSplitLayout
                ...
                left={{
                    ...
                    config: {
                        ...
                        icon: Clock,
                        emptyMessage: 'No hold payments pending.',
                ...
                right={{
                    ...
                    config: {
                        title: 'Hold Payment Details',
                        icon: Lock,
                        selectedTitle: 'Hold Payment Verification',
                        emptyTitle: 'No Payment Selected',
                        emptyMessage: 'Select a hold payment from the list to review and verify.',
                        headerGradient: 'from-rose-50 to-pink-50 dark:from-rose-900/20 dark:to-pink-900/20',
...
export default VerifyHoldPayment;
