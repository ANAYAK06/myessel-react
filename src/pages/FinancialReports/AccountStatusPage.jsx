...
import {
    fetchConsolidateITReportGrid,
    fetchConsolidateITReportDetail,
    setFinancialYear,
    clearFilters,
    resetReportGridData,
    resetReportDetailData,
    clearError,
    selectConsolidateITReportGrid,
    selectConsolidateITReportDetail,
    selectLoading,
    selectErrors,
    selectSelectedFinancialYear,
    selectIsAnyLoading,
    selectHasAnyError
} from '../../slices/financialReportSlice/accountStatusSlice';
...
// Modal Component for IT Detail View
...
// IT Detail Modal Component
const ITDetailModal = ({ isOpen, onClose, itData, detailData, loading }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`IT Code Detail: ${itData?.ITName || 'Unknown'} (${itData?.ITCode || 'N/A'})`} size="full">
            {loading ? (
                <div className="flex items-center justify-center py-8">
                    <RotateCcw className="h-6 w-6 text-indigo-500 animate-spin mr-3" />
                    <p className="text-indigo-700 dark:text-indigo-300">Loading IT code details...</p>
                </div>
            ) : (
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-lg p-4 border border-indigo-200 dark:border-indigo-800">
                        <h4 className="text-lg font-semibold text-indigo-800 dark:text-indigo-200 mb-3">IT Code Summary</h4>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">Cash Payments</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(itData?.GeneralPaymentCash || 0)}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">Bank Payments</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(itData?.GeneralPaymentBank || 0)}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">Gross Expenses</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(itData?.GrossExpenses || 0)}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">Net Expenses</p>
                                <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(itData?.NetExpenses || 0)}</p>
                            </div>
                        </div>
                    </div>
                                ...
                                {details.length > 0 ? details.map((detail, index) => (
                                    ...
                                    <tr>
                                        <td colSpan={columns.length} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                            No detailed transactions available for this IT Code
                                        </td>
                                    </tr>
                                )}
                            </tbody>
...
// Account Overhead (P&L) Summary Component
const AccountOverheadSummary = ({ overheadData }) => {
    const formatCurrency = (amount) => {
        if (!amount && amount !== 0) return '0.00';
        return new Intl.NumberFormat('en-IN', {
            style: 'decimal',
    ...
    const expenseItems = [
        ...
        { label: 'Expenses IT', value: data.ExpensesDCA, icon: Calculator },
...
const AccountStatusPage = () => {
    ...
    // Redux selectors
    const consolidateITReportGrid = useSelector(selectConsolidateITReportGrid);
    const consolidateITReportDetail = useSelector(selectConsolidateITReportDetail);
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            await dispatch(fetchConsolidateITReportGrid(selectedFinancialYear)).unwrap();
            toast.success('Account Status report generated successfully');
    ...
    // Handle IT row click
    const handleITRowClick = async (itData) => {
        try {
            setSelectedITData(itData);
            ...
            const params = {
                ITCode: itData.ITName || itData.ITCode,
            ...
            await dispatch(fetchConsolidateITReportDetail(params)).unwrap();
            
        } catch (error) {
            console.error('❌ Error fetching IT details:', error);
            toast.error('Failed to load IT details');
    ...
    const handleExcelDownload = () => {
        try {
            const data = consolidateITReportGrid?.Data?.ConsolidateITData || [];
            ...
            const filename = `Account_Status_Report_${selectedFinancialYear}_${new Date().toISOString().split('T')[0]}`;
    ...
    const consolidateITData = consolidateITReportGrid?.Data?.ConsolidateITData || [];
    const overheadData = consolidateITReportGrid?.Data?.ITheadData || [];
    ...
    return (
        <div className="space-y-6 p-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700 transition-colors">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                            Comprehensive financial overview with IT code-wise expense analysis and profit & loss summary
                        </p>
                    </div>
                    <div className="flex items-center space-x-3">
                        ...
                        {loading.consolidateITReportGrid && (
                ...
                <nav className="text-sm text-gray-500 dark:text-gray-400">
                    <div className="flex items-center space-x-2">
                        ...
                        <ChevronRight className="w-4 h-4" />
                        <span className="text-gray-900 dark:text-white">Account Status</span>
                    </div>
                </nav>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700 transition-colors">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Tooltip content="Download account status report as Excel file">
                            <button
                                onClick={handleExcelDownload}
                                disabled={!Array.isArray(consolidateITData) || consolidateITData.length === 0}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-colors">
                {Array.isArray(consolidateITData) && consolidateITData.length > 0 ? (
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gradient-to-r from-indigo-600 to-purple-700">
                                <tr>
                                    <th className="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">IT Code</th>
                                    <th className="px-6 py-4 text-left text-xs font-bold text-white uppercase tracking-wider">IT Name</th>
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {consolidateITData.map((item, index) => (
                                    <tr 
                                        key={index} 
                                        className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer"
                                        onClick={() => handleITRowClick(item)}
                                    >
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600 dark:text-indigo-400">
                                            {item.ITName || '-'}
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                            <div className="max-w-xs truncate" title={item.ITName}>
                                                {item.ITCode || '-'}
                                            </div>
                                        </td>
                            ...
                            <div className="p-12 text-center">
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400 max-w-md">
                                        Select a financial year and click "Generate Report" to view the account status report.
                                    </p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="p-12 text-center">
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400">
                                        Fetching account status data for the selected financial year...
                                    </p>
                                </div>
                            </div>
                        )}
            ...
            {/* Account Overhead Summary */}
            <AccountOverheadSummary overheadData={overheadData} />

            {/* IT Detail Modal */}
            <ITDetailModal
                isOpen={isDetailModalOpen}
                onClose={() => setIsDetailModalOpen(false)}
                itData={selectedITData}
                detailData={consolidateITReportDetail}
                loading={loading.consolidateITReportDetail}
            />
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-cyan-50 dark:from-indigo-900/20 dark:to-cyan-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg p-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-indigo-800 dark:text-indigo-200 text-sm">
                        <p className="font-semibold mb-1">Account Status Report Features:</p>
                        <p className="text-gray-600 dark:text-indigo-200">
                            1. <strong>Financial Year Filter:</strong> Select financial year to generate comprehensive account status report<br/>
                            2. <strong>IT Code Analysis:</strong> View detailed expense breakdown by Internal Transfer codes<br/>
                            3. <strong>Profit & Loss Summary:</strong> Complete overhead analysis with income, expenses, and adjustments<br/>
                            4. <strong>Detailed View:</strong> Click any IT code row to view transaction-level details in modal popup<br/>
