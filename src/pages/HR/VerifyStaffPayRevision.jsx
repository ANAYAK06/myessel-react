...
import {
    fetchVerifyPayRevision,
    fetchPayRevisionbyRefno,
    approvePayRevision,
    ...
    clearApprovalResult,
    selectVerifyPayRevisionInboxArray,
    selectPayRevisionDetails,
    selectVerifyPayRevisionLoading,
    selectPayRevisionDetailsLoading,
    selectApprovePayRevisionLoading,
    selectVerifyPayRevisionError,
...
const VerifyStaffPayRevision = ({ notificationData, onNavigate }) => {
    const dispatch = useDispatch();

    // Selectors
    const revisionInbox = useSelector(selectVerifyPayRevisionInboxArray);
    const inboxLoading = useSelector(selectVerifyPayRevisionLoading);
    const inboxError = useSelector(selectVerifyPayRevisionError);
    ...
    const approvalLoading = useSelector(selectApprovePayRevisionLoading);
    ...
    const categories = [...new Set(revisionInbox.map(item => item.Category))].filter(Boolean);
    ...
    useEffect(() => {
        if (roleId) {
            console.log('🎯 Initializing Staff Pay Revision Verification with RoleID:', roleId);
            dispatch(setSelectedRoleId(roleId));
            dispatch(fetchVerifyPayRevision(roleId));
        }
    }, [roleId, dispatch]);
    ...
    useEffect(() => {
        ...
        return () => {
            dispatch(resetStaffPayRevisionData());
    ...
    useEffect(() => {
        if (selectedItem?.TransactionRefno) {
            console.log('🔍 Fetching Staff Pay Revision Details for TransactionRefno:', selectedItem.TransactionRefno);

            dispatch(setSelectedTransactionRefno(selectedItem.TransactionRefno));
            dispatch(fetchPayRevisionbyRefno(selectedItem.TransactionRefno));
    ...
    const handleRefresh = () => {
        if (roleId) {
            console.log('🔄 Refreshing Staff Pay Revision list');
            dispatch(fetchVerifyPayRevision(roleId));
    ...
    const buildApprovalPayload = (actionValue) => {
        ...
        const empRefNo = revisionDetails?.EmpRefNo || selectedItem?.Emprefno || '';
        ...
        console.log('📤 Staff Pay Revision Approval Payload:', payload);
        return payload;
    };
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                            {revisionDetails.PayRevisionHeadData.EmpRuleStatus.PFExist && (
                                <div>
                                    ...
                                    <p className="font-semibold text-gray-900 dark:text-white">
                                        {revisionDetails.PayRevisionHeadData.EmpRuleStatus.PFExist} ({revisionDetails.PayRevisionHeadData.EmpRuleStatus.PFPercent}%)
                                    </p>
                                </div>
                            )}
                            {revisionDetails.PayRevisionHeadData.EmpRuleStatus.ESIExist && (
                                <div>
                                    <p className="text-gray-500 dark:text-gray-400">ESI Status</p>
                                    <p className="font-semibold text-gray-900 dark:text-white">
                                        {revisionDetails.PayRevisionHeadData.EmpRuleStatus.ESIExist} ({revisionDetails.PayRevisionHeadData.EmpRuleStatus.ESIPercent}%)
                                    </p>
                                </div>
                            )}
                            {revisionDetails.PayRevisionHeadData.EmpRuleStatus.GratuityRuleExist && (
                                <div>
                            ...
                            {revisionDetails.PayRevisionHeadData.EmpRuleStatus.PaidLeaveExist && (
                                <div>
                        ...
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-blue-600 dark:text-blue-400 font-semibold mb-3">
                                    {hasDetailedData ? displayData.EmpRefNo : displayData.Emprefno} • {hasDetailedData ? displayData.TransactionRefNo : displayData.TransactionRefno}
                                </p>
    ...
    return (
        <div className="space-y-6">
            <InboxHeader
                ...
                searchConfig={{
                    enabled: true,
                    placeholder: 'Search by name, employee code, transaction ref...',
                    value: searchQuery,
            ...
            <InboxSplitLayout
                ...
                right={{
                    ...
                    config: {
                        title: 'Pay Revision Details',
                        icon: RefreshCw,
                        selectedTitle: 'Pay Revision Verification',
                        emptyTitle: 'No Pay Revision Selected',
                        emptyMessage: 'Select a pay revision record from the list to view details and verify changes.',
                        headerGradient: 'from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20',
...
export default VerifyStaffPayRevision;